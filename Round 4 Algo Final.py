...
class Trader:
    ...
    def __init__(self):
        ...
        self.macarons_trader = MacaronsTrader()
    ...
    def run(self, state: TradingState):
        ...
        mac_orders, mac_convs, mac_traderData = self.macarons_trader.run(state)
        for prod, orders in mac_orders.items():
            result.setdefault(prod, []).extend(orders)
        conversions = 1 + mac_convs
        state.traderData = mac_traderData
...
class MacaronsTrader:
    PRODUCT = "MAGNIFICENT_MACARONS"

    def __init__(self):
        self.pos_limit        = 75
        self.sun_entry_thresh = 50.0
        self.sun_exit_grad    = 0.000075
        self.checkpoint_int   = 100000
    ...
    def run(self, state: TradingState) -> Tuple[dict, int, str]:
        ...
        # unpack our per-product data (stored as a simple CSV in traderData[product])
        td_dict = {}
        if state.traderData:
            td_dict = jsonpickle.decode(state.traderData)
        raw = td_dict.get(product, "")
        parts = raw.split(',') if raw else []
        last_sun_ts   = int(parts[0]) if len(parts) >= 1 and parts[0] else state.timestamp
        last_sun_val  = float(parts[1]) if len(parts) >= 2 and parts[1] else 0.0
        phase         = parts[2] if len(parts) >= 3 and parts[2] else 'normal'
        trigger_price = float(parts[3]) if len(parts) >= 4 and parts[3] else 0.0
        ...
        sun  = conv.sunlightIndex if conv is not None else last_sun_val

        # update checkpoint & gradient
        if ts % self.checkpoint_int == 0:
            last_sun_ts, last_sun_val = ts, sun
        dt = ts - last_sun_ts
        grad = (sun - last_sun_val) / dt if dt > 0 else 0.0

        # decide target position & phase
        target = 0
        new_phase = phase
        new_trigger = trigger_price

        if phase == 'normal':
            if sun < self.sun_entry_thresh:
                target, new_phase, new_trigger = self.pos_limit, 'sun', mid_price
        elif phase == 'sun':
            if grad > self.sun_exit_grad:
                target, new_phase = -self.pos_limit, 'short'
            else:
                target = self.pos_limit
        else:  # short
            if mid_price <= trigger_price:
                target, new_phase, new_trigger = 0, 'normal', 0.0
            else:
                target = -self.pos_limit

        # send one order towards target
        pos = state.position.get(product, 0)
        delta = target - pos
        if delta > 0:
            orders.append(Order(product, int(best_ask), delta))
        elif delta < 0:
            orders.append(Order(product, int(best_bid), delta))

        # persist new data
        new_raw = f"{last_sun_ts},{last_sun_val},{new_phase},{new_trigger}"
        td_dict[product] = new_raw
        new_traderData = jsonpickle.encode(td_dict)
        return ({product: orders} if orders else {}), conversions, new_traderData
